...
from app.services.excel_import import parse_buildings_excel, validate_buildings_data, save_buildings_from_validated

router = APIRouter(prefix="/buildings", tags=["Buildings"])

@router.get("/", response_model=list[BuildingResponse])
async def read_buildings(db: AsyncSession = Depends(get_db)):
    return await building_service.get_all_buildings(db)
...
async def upload_buildings(
    ...
    try:
        ...
        rows = await parse_buildings_excel(content)
    except Exception as e:
        raise HTTPException(400, f"Ошибка чтения файла: {str(e)}")

    valid_rows, errors = await validate_buildings_data(rows, db)
    result = await save_buildings_from_validated(valid_rows, db)
